defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.claude.command == "claude-app-server"
    ...
    write_workflow_file!(Workflow.workflow_file_path(), claude_command: "codex app-server --model gpt-5.3-codex")
    assert Config.settings!().claude.command == "codex app-server --model gpt-5.3-codex"
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      claude_command: "#{agent_bin} app-server"
    )

    config = Config.settings!()
    assert config.tracker.api_key == api_key
    assert config.workspace.root == Path.expand(workspace_root)
    assert config.claude.command == "#{agent_bin} app-server"
  end
