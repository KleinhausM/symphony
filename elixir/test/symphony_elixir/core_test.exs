defmodule SymphonyElixir.CoreTest do
  ...
  test "agent runner keeps workspace after successful agent run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
  ...
  test "app server startup command supports agent args override from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        workspace_root: workspace_root,
        claude_command: "#{agent_binary} --model gpt-5.3-codex app-server"
      )

      issue = %Issue{
        id: "issue-custom-args",
        identifier: "MT-88",
        title: "Validate custom agent args",
  ...
  test "app server startup payload uses configurable approval and sandbox settings from workflow config" do
    ...
    try do
      ...
      issue = %Issue{
        ...
        identifier: "MT-99",
        title: "Validate agent policy overrides",
