...
- `Human Review` -> implementation complete and validated; waiting on human to review and merge.
...
1.  Determine current repo state (`branch`, `git status`, `HEAD`) and record it in the workpad before implementation continues.
...
5.  Run validation/tests required for the scope.
    ...
    - Revert every temporary proof edit before finishing.
    - Document these temporary proof steps and outcomes in the workpad `Validation`/`Notes` sections so reviewers can follow the evidence.
6.  Re-check all acceptance criteria and close any gaps.
7.  Update the workpad comment with final checklist status and validation notes.
    - Mark completed plan/acceptance/validation checklist items as checked.
    - Add final handoff notes (validation summary) in the same workpad comment.
    - Add a short `### Confusions` section at the bottom when any part of task execution was unclear/confusing, with concise bullets.
    - Do not post any additional completion summary comment.
8.  Confirm every required ticket-provided validation/test-plan item is explicitly marked complete in the workpad.
9.  Re-open and refresh the workpad before state transition so `Plan`, `Acceptance Criteria`, and `Validation` exactly match completed work.
...
3. Remove the existing `## Workpad` comment from the issue.
4. Start over from the normal kickoff flow:
...
- Validation/tests are green.
- If app-touching, runtime validation requirements are complete.
...
- Temporary proof edits are allowed only for local verification and must be reverted before finishing.
